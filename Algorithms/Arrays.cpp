...
{
	// This would be an efficient implementation of the quicksort partitioning step.
...
void PermuteArray(vector<int> a, const vector<int>& p)
{
...
// ----------------------------------------------------------
// 6.10 COMPUTE THE NEXT PERMUATION
...
{
	// Initialize an array with a default increasing of n permutation values, than use solution 6.11 to sample from it, return a subsample of size n.
...
void RotateMatrix(vector<vector<int>>& m)
{
	...
	// Another contrived hack would be to rotate the values on read. The problem specifiically states we should rotate the array however, so this is more trivia than an actual solution.
}
